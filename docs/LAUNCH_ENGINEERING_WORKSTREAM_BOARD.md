...
- The A2 weekly readable-copy repair is `accepted` as public Beta trust/readability support: `src/app/weekly/page.tsx` now uses reader-facing weekly report copy, keeps `publicDataSource=mock`, keeps `scoreSource=mock`, states `mock-only`, `data freshness metadata`, and `not investment advice`, and remains guarded by `check:a2-weekly-readable-copy`.
