...
接受董事長最新版 BRIEF，並切成兩個階段執行：

- Phase 1：所有人都可以使用的免費「指數燈號網站」。
- Phase 2：會員 MVP 與個人化追蹤產品。
- 目前主線優先完成 Phase 1。
- Phase 2 很重要，但不得拖慢 Phase 1 公開 Beta 可用性。
...
CEO 判斷：Phase 1 先完成免費公開市場總覽、核心指標、主要風險提示與資料更新時間。會員登入、會員深度解讀、自選追蹤、自訂警示、盤後複盤與付款流程屬於 Phase 2；這些能力先保留為清楚的 roadmap / preview，不作為 Phase 1 上線阻塞。

## 1. 目標

本專案目標是打造一個面向一般投資者的「指數燈號網站」，透過紅、黃、綠等視覺化狀態，協助使用者快速理解目前市場風險、趨勢強弱與觀察重點。

第一階段以「降低市場資訊理解門檻」為核心，讓使用者在短時間內看懂目前市場處於偏多、觀望、警戒或高風險狀態。
...
如果網站只提供免費燈號與單層數據，長期容易停留在「查詢工具」定位，使用頻率與商業價值有限。因此下一階段需要規劃會員內容，將「看到燈號」延伸為「理解燈號」、「追蹤變化」與「建立自己的觀察紀錄」。
...
- 會員自選追蹤 + 自訂警示條件：讓使用者追蹤自己關心的指數、ETF 或指標。
...
### Phase 1：所有人可使用的免費指數燈號網站

Phase 1 是目前主線，也是公開 Beta 的上線目標。
...
- 核心頁面可正常載入，RWD 正常，可讀性符合公開 Beta 基準。
- 所有頁面皆有必要的風險聲明與非投資建議提示。
- 資料來源、更新時間、延遲或示範資料邊界清楚。
...
- Phase 1 免費公開產品與 runtime 整合
- Home / briefing / weekly / stock / methodology / disclaimer / terms / privacy 的公開可讀性
- 燈號、狀態、原因、更新時間、觀察重點、資料邊界的一致呈現
- 公開 Beta 上線判斷
...
- 自選追蹤 / alert / post-market review 的資料模型與產品流程
- 商業指標與轉換事件
- 不進入實作，直到 CEO/PM 判定 Phase 1 穩定

CEO 可隨時調整比例，但預設在 Phase 1 尚未穩定前：
...
1. 先完成 Phase 1 免費公開指數燈號網站，不讓會員 MVP 拖慢上線。
2. 移除公開頁面中所有使用者不該看到的開發殘留、內部命令、packet、operator、raw payload、local path、PM/CEO/A1/A2 字樣。
3. 讓 Home / briefing / weekly / stock / methodology / disclaimer / terms / privacy 支援 30 秒市場氣氛判讀與 3 分鐘觀察判斷。
4. A1 繼續資料真實化與資料覆蓋，但在資料權利與自動化來源完全確認前，不阻塞 Phase 1 mock public Beta。
5. A2 / A3 / A4 只做能加速 Phase 1 或降低上線風險的支援工作。
