...
const docPath = "docs/PHASE_1_DATA_ONLINE_EXTERNAL_PLATFORM_EVIDENCE_RUNNER_NO_EXECUTION.md";
...
const checklist = runJson(
  "scripts/check-phase-1-data-online-external-platform-evidence-checklist-no-execution.mjs",
  "external-platform evidence checklist"
);
...
const report = {
  ...
  guardedStatus: ok
    ...
    : "phase_1_data_online_external_platform_evidence_runner_no_execution_blocked",
  packetMode: "external_platform_evidence_runner_no_execution",
  externalPlatformEvidenceGathered: false,
  externalPlatformEvidencePending: pendingEvidenceItems,
...
function validatePrerequisites() {
  expect(checklist.status, "ok", "external-platform checklist status");
  expect(
    checklist.guardedStatus,
    "phase_1_data_online_external_platform_evidence_checklist_no_execution_ready",
    "external-platform checklist guarded status"
  );
  expect(checklist.externalPlatformEvidenceReady, true, "external-platform checklist externalPlatformEvidenceReady");
  expect(checklist.writeGateExecutableNow, false, "external-platform checklist writeGateExecutableNow");
...
function validateDoc() {
  const requiredTokens = [
    "phase_1_data_online_external_platform_evidence_runner_no_execution_ready",
    "external_platform_evidence_runner_no_execution",
...
function validateRegistration() {
  if (
    packageJson.scripts?.["check:phase-1-data-online-external-platform-evidence-runner-no-execution"] !==
    "node scripts/check-phase-1-data-online-external-platform-evidence-runner-no-execution.mjs"
  ) {
    problems.push("package.json missing check:phase-1-data-online-external-platform-evidence-runner-no-execution");
  }
  if (!reviewGate.includes("scripts/check-phase-1-data-online-external-platform-evidence-runner-no-execution.mjs")) {
    problems.push("review gate missing external-platform evidence runner checker");
  }
  if (!reviewGate.includes('"phase-1-data-online-external-platform-evidence-runner-no-execution"')) {
    problems.push("focused review gate missing external-platform evidence runner checker");
  }
}
...
function validateBoundaries() {
  const forbidden = [
    ...
    "externalPlatformEvidenceGathered=true",
