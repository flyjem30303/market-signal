...
const requiredRunnerPhrases = [
  writeImplementationCreated ? "tw_equity_staging_write_fail_closed_write_capable_runner" : "tw_equity_staging_write_fail_closed_runner_skeleton",
...
if (writeImplementationCreated) {
  requiredRunnerPhrases.push("writeImplementationReady: true", "executeBoundedStagingWrite");
} else {
  requiredRunnerPhrases.push("runner_skeleton_has_no_supabase_write_implementation", "writeImplementationReady: false");
}
...
if (executeAttempt.status === 0) {
  ...
  if (writeImplementationCreated) {
    if (parsed.executionAttempted !== false) problems.push("--execute without candidate must not attempt execution");
    if (parsed.writeImplementationReady !== true) problems.push("--execute output must report writeImplementationReady true after implementation");
...
function validateDryRun(report) {
  const expected = {
    ...
    writeImplementationReady: writeImplementationCreated
  };
