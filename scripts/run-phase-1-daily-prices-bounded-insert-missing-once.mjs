...
const EXPECTED = {
  authorizationId: "PHASE1-DAILY-PRICES-BOUNDED-WRITE-2026-06-16-A",
  acknowledgeFlag: "CEO_AUTHORIZED_ONE_PHASE1_BOUNDED_WRITE_ATTEMPT_20260616_A",
  candidateRows: 178,
  candidateScope: "twii_and_etf_phase_1_missing_row_closure_only",
  maxRows: 178,
  symbols: ["0050", "006208", "TWII"],
...
const executionResult =
  executionRequested && problems.length === 0
    ? await executeBoundedInsertMissing(candidateValidation.artifact)
    : skippedExecution();
...
if (executionRequested) writePostRunReview(args.postRunReview, output);
...
function validateCandidateArtifact(filePath) {
  ...
  if (rows.length !== EXPECTED.candidateRows) validation.problems.push("candidate_row_count_mismatch");
...
async function executeBoundedInsertMissing(artifact) {
  const result = {
  ...
  const stockIdBySymbol = await lookupStockIdsBySymbol(supabase);
  result.stockMappingComplete = stockIdBySymbol.size === EXPECTED.symbols.length;
  if (!result.stockMappingComplete) {
    result.problems.push("stock_mapping_incomplete");
    return result;
  }

  const rowsBySymbol = groupRowsBySymbol(artifact.rows);
  ...
  result.missingRowsAfterWrite = EXPECTED.candidateRows - finalKeys.size;
...
async function lookupStockIdsBySymbol(supabase) {
  const { data, error } = await supabase
    .from("stocks")
    .select("id,symbol")
    .eq("country", "TW")
    .eq("exchange", "TWSE")
    .in("symbol", EXPECTED.symbols);

  if (error || !Array.isArray(data)) return new Map();
  const ids = new Map();
  for (const row of data) {
    if (typeof row?.id === "string" && EXPECTED.symbols.includes(row?.symbol)) {
      ids.set(row.symbol, row.id);
    }
  }
  return ids;
}
...
async function readExistingKeys(supabase, stockIdBySymbol, rowsBySymbol, result) {
  ...
  for (const symbol of EXPECTED.symbols) {
    ...
    const tradeDates = rowsBySymbol.get(symbol)?.map((row) => row.trade_date) ?? [];
...
function groupRowsBySymbol(rows) {
  const grouped = new Map(EXPECTED.symbols.map((symbol) => [symbol, []]));
...
function skippedExecution() {
  return {
    ...
    finalRowsAfterWrite: 0,
    insertedRows: 0,
    missingRowsAfterWrite: EXPECTED.candidateRows,
    plannedInsertRows: 0,
    problems: [],
    readbackAttempted: false,
  ...
  return {
    status: passed
      ? executionRequested
        ...
        : "phase_1_daily_prices_bounded_insert_missing_ready_not_executed"
      : "phase_1_daily_prices_bounded_insert_missing_blocked",
    mode: "phase_1_daily_prices_bounded_insert_missing_once",
    ...
    symbolsCovered: candidateValidation.aggregate?.symbolsCovered ?? [],
    symbolCounts: candidateValidation.aggregate?.symbolCounts ?? null,
