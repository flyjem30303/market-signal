...
const outputPath = path.normalize(process.env.PUBLIC_BETA_EXTERNAL_REPLY_ENV_TEMPLATE_PATH || "tmp/public-beta-external-reply-env.template.ps1");
...
const lines = [
  "# Public Beta external reply local env template.",
  "# Fill only no-secret values. Do not paste Supabase keys, raw payloads, row payloads, copied contract text, or private links.",
  "# After filling this file, run it in PowerShell, then run:",
  `#   ${fillCommand}`,
  `#   ${routeCommand}`,
  "",
  ...requiredEnvVars.map(([name, value]) => `$env:${name}='${value}'`)
];

if (fs.existsSync(outputPath)) {
  console.log(JSON.stringify({
    status: "public_beta_external_reply_env_template_blocked_existing_file",
    ok: false,
    mode: "public_beta_external_reply_env_template_writer",
    outputPath,
    fileWritten: false,
    fileOverwritten: false,
    fileTextEchoed: false,
    valuesEchoed: false,
    nextAction: "Edit or move the existing ignored env template file; it was not overwritten.",
...
console.log(JSON.stringify({
  status: "public_beta_external_reply_env_template_written",
  ok: true,
  mode: "public_beta_external_reply_env_template_writer",
  outputPath,
  ...
  fileTextEchoed: false,
  valuesEchoed: false,
  nextLocalFileAction: `Fill and run ${outputPath} in PowerShell`,
