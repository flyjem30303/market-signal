...
const recordPath = "scripts/record-narrow-approval-outcome.mjs";
...
for (const phrase of [
  "local_narrow_approval_outcome_recording",
...
if (packageJson.scripts?.["record:narrow-approval-outcome"] !== "node scripts/record-narrow-approval-outcome.mjs") {
  missing.push(`${packagePath}: record:narrow-approval-outcome`);
}

if (packageJson.scripts?.["check:record-narrow-approval-outcome"] !== "node scripts/check-record-narrow-approval-outcome.mjs") {
  missing.push(`${packagePath}: check:record-narrow-approval-outcome`);
}

if (!reviewGate.includes("scripts/check-record-narrow-approval-outcome.mjs")) {
  missing.push(`${reviewGatePath}: scripts/check-record-narrow-approval-outcome.mjs`);
}

if (!fullHealth.includes("scripts/check-record-narrow-approval-outcome.mjs")) {
  missing.push(`${fullHealthPath}: scripts/check-record-narrow-approval-outcome.mjs`);
}
...
const run = spawnSync(
  ...
  [
    ...
    "--note",
    "Dry run verifies accepted outcome recording without mutating the ledger."
  ],
  {
    cwd: process.cwd(),
    encoding: "utf8",
...
if (output) {
  if (output.mode !== "local_narrow_approval_outcome_recording") {
    blocked.push(`output.mode: ${String(output.mode)}`);
  }
  if (output.status !== "dry_run") {
    blocked.push(`output.status: ${String(output.status)}`);
  }
  if (output.target !== "legal-source-terms-review" || output.requestedOutcome !== "accepted") {
  ...
  if (!Array.isArray(output.stillDoesNotAuthorize) || output.stillDoesNotAuthorize.length < 8) {
    blocked.push("output.stillDoesNotAuthorize: expected at least 8 items");
  }
}
