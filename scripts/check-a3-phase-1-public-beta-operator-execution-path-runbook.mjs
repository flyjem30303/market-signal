...
const docPath = "docs/A3_PHASE_1_PUBLIC_BETA_OPERATOR_EXECUTION_PATH_RUNBOOK.md";
...
const requiredPhrases = [
  ...
  "follow_no_secret_manual_platform_action_checklist_with_accepted_deferrals",
...
for (const decisionOption of ["`GO`", "`GO_WITH_DEFERRALS`", "`NO_GO`"]) {
  if (!doc.includes(decisionOption)) problems.push(`${docPath} missing decision option: ${decisionOption}`);
}
...
for (const artifact of [
  ...
  if (!doc.includes(artifact)) problems.push(`${docPath} missing artifact reference: ${artifact}`);
}

for (const lane of ["PM", "A1", "A2", "A3", "A4"]) {
  if (!doc.includes(`${lane}:`)) problems.push(`${docPath} missing rejected-route owner lane: ${lane}`);
}
...
if (!decision.includes("operator_follows_phase_1_public_beta_manual_platform_action_checklist_or_pm_repairs_blocker")) {
  problems.push(`${decisionPath} missing operator execution route linkage`);
}
...
if (!postPlatform.includes("no development residue visible")) {
  problems.push(`${postPlatformPath} missing post-platform development residue guard`);
}

if (!keepOpen.includes("public visible residue cleanup passes")) {
  problems.push(`${keepOpenPath} missing keep-open public residue condition`);
}

if (
  pkg.scripts?.["check:a3-phase-1-public-beta-operator-execution-path-runbook"] !==
  "node scripts/check-a3-phase-1-public-beta-operator-execution-path-runbook.mjs"
) {
  problems.push(`${packagePath} missing check:a3-phase-1-public-beta-operator-execution-path-runbook script`);
}

if (!reviewGate.includes("scripts/check-a3-phase-1-public-beta-operator-execution-path-runbook.mjs")) {
  problems.push(`${reviewGatePath} missing operator execution path checker`);
}
...
console.log(
  JSON.stringify(
    {
      status: "ok",
      guardedStatus: "a3_phase_1_public_beta_operator_execution_path_runbook_ready",
