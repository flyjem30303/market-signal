...
if (output.nextReviewOnlyRoute !== "explicit_execution_packet_preparation_review_then_separate_authorized_execution_attempt_preparation") problems.push("nextReviewOnlyRoute mismatch");
if (output.allowedNextCommandCategory !== "review_only_separate_authorized_execution_attempt_preparation") problems.push("allowedNextCommandCategory mismatch");
if (output.executionPacketValidation?.requiredExecutionPacketFieldCount !== 13) problems.push("requiredExecutionPacketFieldCount must be 13");
if (output.executionPacketValidation?.placeholderCount !== 13) problems.push("placeholderCount must be 13");
if (output.executionPacketValidation?.fieldNameOnlyPlaceholderCount !== 13) problems.push("fieldNameOnlyPlaceholderCount must be 13");
if (output.executionPacketValidation?.presenceOnlyPlaceholderCount !== 13) problems.push("presenceOnlyPlaceholderCount must be 13");
...
for (const key of ["explicitExecutionPacketPreparationGatePrepared", "boundedOperatorAuthorizationPacketPreparationReferenced", "serverOnlyPreExecutionChecksReferenced", "rollbackContractReferenced", "aggregateReadbackContractReferenced", "postRunReviewContractReferenced", "boundedInsertContractReferenced", "explicitExecutionPacketShapePrepared", "requiredExecutionPacketFieldsPrepared", "operatorAuthorizationPacketHandoffPrepared", "operatorDecisionPresencePrepared", "authorizationPresencePrepared", "executeSwitchPresencePrepared", "confirmationPhrasePresencePrepared", "serverOnlyCredentialPresencePrepared", "rollbackDryRunPlaceholderPrepared", "aggregateReadbackPlaceholderPrepared", "postRunReviewPlaceholderPrepared", "candidateDuplicateRejectionPlaceholderPrepared", "mockBoundaryRechecked", "executionStopLinesPrepared", "reviewOnly", "localOnly", "shapeOnly", "presenceOnly", "fieldNameOnly", "serverOnly"]) if (output.executionPacketState?.[key] !== true) problems.push(`executionPacketState.${key} must be true`);
for (const key of ["externalOnlyValuesProvidedNow", "externalOperatorDecisionProvidedNow", "operatorAuthorizationAcceptedNow", "authorizationValueReadNow", "serverOnlyCredentialCheckPassed", "executeSwitchProvided", "confirmationPhraseProvided", "runnerExecutableNow", "executionAllowedNow", "writeGateExecutableNow", "finalExecutionAllowedNow", "implementationAllowedNow"]) if (output.executionPacketState?.[key] !== false) problems.push(`executionPacketState.${key} must be false`);
