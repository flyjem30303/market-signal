...
const required = [
  ...
  [helperPath, "official open-data routes accepted locally"],
  [helperPath, "Phase 1 scope is TWII plus listed-stock daily close"],
  [helperPath, "ETF coverage is deferred to Phase 1.1"],
  [helperPath, "keep redistribution, export, and API reuse blocked"],
  ...
  [queuePath, "source-depth accepted for Phase 1 TWII plus listed-stock daily close"],
...
const staticAcceptedOutcomeCount = (read(helperPath).match(/outcome: "accepted",/g) ?? []).length;
if (staticRejectedOutcomeCount !== 0) {
  blocked.push({ file: helperPath, phrase: `expected 0 static rejected outcomes, found ${staticRejectedOutcomeCount}` });
}
if (staticAcceptedOutcomeCount !== 1) {
  blocked.push({ file: helperPath, phrase: `expected 1 static accepted source-depth outcome, found ${staticAcceptedOutcomeCount}` });
}
