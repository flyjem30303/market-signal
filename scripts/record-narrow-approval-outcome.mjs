...
const outcomeDataPath = path.join(process.cwd(), "data/source-gates/narrow-approval-outcomes.json");
const allowedIds = new Set(["legal-source-terms-review", "investment-non-advisory-interpretation-review"]);
...
validateInput({ id, outcome, recordedAt, recordedBy, decisionNote });
...
const result = {
  mode: "local_narrow_approval_outcome_recording",
...
function validateInput({ id, outcome, recordedAt, recordedBy, decisionNote }) {
...
function defaultDecisionNote(id, outcome) {
  ...
  return `Outcome ${outcome} recorded for ${id} by local CEO-controlled process.`;
}
