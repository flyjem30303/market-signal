...
const CONFIRMATION_VALUE = "CEO_APPROVED_TW_EQUITY_POST_MIGRATION_OPENAPI_EXPOSURE_CONFIRMATION_ONCE";
const DOTENV_LOCAL_ALLOWED_KEYS = ["NEXT_PUBLIC_SUPABASE_URL", "SUPABASE_SERVICE_ROLE_KEY"];
const READONLY_REVIEW_PATH =
...
const confirmationAccepted =
  process.env.TW_EQUITY_POST_MIGRATION_OPENAPI_EXPOSURE_CONFIRMATION === CONFIRMATION_VALUE;
const readonlyReviewAccepted = loadReadonlyReviewAccepted();
...
if (!confirmationAccepted) {
  finish({
    ...
    status: "tw_equity_post_migration_openapi_exposure_confirmation_not_run_confirmation_required"
  });
} else if (!readonlyReviewAccepted) {
  finish({
    connectionAttempted: false,
    exposure: notRunExposure("post_migration_readonly_verification_required"),
    postRunReviewWritten: false,
    status: "tw_equity_post_migration_openapi_exposure_confirmation_blocked_readonly_review_required"
  });
  ...
  finish({
    ...
    postRunReviewWritten: false,
    status: "tw_equity_post_migration_openapi_exposure_confirmation_blocked_missing_credentials"
  });
} else {
...
function classify(exposure) {
  if (!exposure.openApiReachable) return "tw_equity_post_migration_openapi_exposure_confirmation_openapi_not_reachable";
  if (!exposure.openApiParsed) return "tw_equity_post_migration_openapi_exposure_confirmation_openapi_not_parseable";
  if (exposure.problems.length > 0) {
    return "tw_equity_post_migration_openapi_exposure_confirmation_schema_exposure_incomplete_write_blocked";
  }
  return "tw_equity_post_migration_openapi_exposure_confirmation_schema_exposure_complete_write_path_ready_for_decision";
}
...
function writePostRunReview({ exposure, status }) {
  ...
  const lines = [
    ...
    `- Post-migration readonly verification observed: \`${readonlyReviewAccepted ? "accepted" : "not_accepted"}\`.`,
    "",
    "## Sanitized Result",
    "",
    ...
    "## Decision",
    "",
    exposure.problems.length === 0
      ? "- REST/OpenAPI exposure is complete enough to prepare a separate bounded staging write decision."
      : "- REST/OpenAPI exposure remains incomplete; bounded staging write remains blocked until schema exposure is repaired or explained.",
    "",
    "## Safety Confirmation",
    "",
...
function loadReadonlyReviewAccepted() {
  if (!fs.existsSync(READONLY_REVIEW_PATH)) return false;
  const review = fs.readFileSync(READONLY_REVIEW_PATH, "utf8");
  return (
    review.includes("tw_equity_post_migration_readonly_verification_tables_reachable_no_write") &&
    review.includes("`staging_twse_stock_day_runs`: reachable=`ok`") &&
    review.includes("`staging_twse_stock_day_prices`: reachable=`ok`")
  );
}
...
function finish({ connectionAttempted, exposure, postRunReviewWritten, status }) {
  console.log(
    JSON.stringify(
      {
        status,
        mode: "tw_equity_post_migration_openapi_exposure_confirmation_once",
        confirmation: confirmationAccepted ? "present" : "missing_or_invalid",
        readonlyReviewGate: {
          accepted: readonlyReviewAccepted,
          path: READONLY_REVIEW_PATH,
          requiredStatus: "tw_equity_post_migration_readonly_verification_tables_reachable_no_write"
        },
        credentialPresence,
        connectionAttempted,
        exposure,
