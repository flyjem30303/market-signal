...
const reportPath = "scripts/report-twii-final-runtime-execution-gate-preflight.mjs";
const docPath = "docs/TWII_FINAL_RUNTIME_EXECUTION_GATE_PREFLIGHT.md";
...
const output = parseJson(run.stdout ?? "", "final runtime execution gate stdout");
if (run.status !== 0) problems.push("final runtime execution gate report must exit 0");
if (output.status !== "twii_final_runtime_execution_gate_preflight_ready_no_execution") {
  problems.push("final runtime execution gate status mismatch");
}
if (output.outcome !== "final_runtime_execution_gate_ready_execution_still_blocked") {
  problems.push("final runtime execution gate outcome mismatch");
}
if (output.mode !== "twii_final_runtime_execution_gate_preflight_no_execution") {
  problems.push("final runtime execution gate mode mismatch");
}
if (output.runtimeGateMode !== "final_runtime_execution_gate_preflight_fail_closed_no_execution") {
  problems.push("runtimeGateMode mismatch");
}
...
assertGate(gate);
assertRuntimeExecutionGateState(output.runtimeExecutionGateState ?? {});
...
if (pkg.scripts?.["report:twii-final-runtime-execution-gate-preflight"] !== `node ${reportPath}`) {
  problems.push(`${packagePath} missing report:twii-final-runtime-execution-gate-preflight`);
}
if (
  pkg.scripts?.["check:twii-final-runtime-execution-gate-preflight"] !==
  "node scripts/check-twii-final-runtime-execution-gate-preflight.mjs"
) {
  problems.push(`${packagePath} missing check:twii-final-runtime-execution-gate-preflight`);
}

for (const phrase of [
  "TWII Final Runtime Execution Gate Preflight",
  "twii_final_runtime_execution_gate_preflight_ready_no_execution",
  "final_runtime_execution_gate_ready_execution_still_blocked",
  "data/source-gates/twii-final-runtime-execution-gate-preflight.json",
...
for (const phrase of [
  "Latest TWII final runtime execution gate preflight slice",
  "docs/TWII_FINAL_RUNTIME_EXECUTION_GATE_PREFLIGHT.md",
  "data/source-gates/twii-final-runtime-execution-gate-preflight.json",
  "twii_final_runtime_execution_gate_preflight_ready_no_execution",
  "final_runtime_execution_gate_ready_execution_still_blocked"
]) {
...
for (const phrase of [
  "`docs/TWII_FINAL_RUNTIME_EXECUTION_GATE_PREFLIGHT.md` is `accepted` as TWII final runtime execution gate preflight",
  "twii_final_runtime_execution_gate_preflight_ready_no_execution",
  "final_runtime_execution_gate_ready_execution_still_blocked"
]) {
...
for (const phrase of [
  "scripts/check-twii-final-runtime-execution-gate-preflight.mjs",
...
for (const [filePath, text] of [
  ...
  [docPath, doc],
  [gatePath, gateText],
  ["final runtime execution gate stdout", run.stdout ?? ""]
]) {
  ...
  JSON.stringify(
    {
      ...
      runtimeGateMode: output.runtimeGateMode,
      runnerExecutableNow: output.runtimeExecutionGateState.runnerExecutableNow,
      executionAllowedNow: output.runtimeExecutionGateState.executionAllowedNow,
...
function assertRuntimeExecutionGateState(state) {
  for (const key of [
    "runtimeExecutionGatePrepared",
    "executionReviewReferenced",
    "serverOnlyBoundaryReferenced",
    "failClosedDefaultPrepared",
    ...
    if (state[key] !== true) problems.push(`runtimeExecutionGateState.${key} must be true`);
  }
  for (const key of ["runnerExecutableNow", "executionAllowedNow", "writeGateExecutableNow", "implementationAllowedNow"]) {
    if (state[key] !== false) problems.push(`runtimeExecutionGateState.${key} must be false`);
  }
}

function assertAuthorizationValuesState(state) {
  for (const key of ["authorizationValuesRead", "executeSwitchValueRead", "confirmationPhraseValueRead", "credentialValuesRead", "envValueOutput"]) {
