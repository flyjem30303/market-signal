...
const problems = [];
const reportPath = "scripts/report-twii-pre-execution-stopline-chain-convergence-gate.mjs";
const docPath = "docs/TWII_PRE_EXECUTION_STOPLINE_CHAIN_CONVERGENCE_GATE.md";
...
const output = parseJson(run.stdout ?? "", "pre-execution stopline chain convergence stdout");

if (run.status !== 0) problems.push("pre-execution stopline chain convergence report must exit 0");
if (output.status !== "twii_pre_execution_stopline_chain_convergence_gate_ready_no_execution") problems.push("status mismatch");
if (output.outcome !== "pre_execution_stopline_chain_converged_execution_still_blocked") problems.push("outcome mismatch");
if (output.nextPMRoute !== "twii_external_values_shape_recheck_preparation_gate") problems.push("nextPMRoute mismatch");
if (output.chain?.readyGateCount !== 7) problems.push("readyGateCount must be 7");
...
for (const key of [
  "explicitOperatorPacketPreparationReady",
  "separateAttemptPreparationReady",
  "finalStoplinePreparationReady",
  "explicitGoNoGoAlignmentReady",
  "explicitGoNoGoDecisionPreparationReady",
  "operatorValueIntakeStoplineReady",
  "externalValuesShapeRecheckPreparedAsNextRoute",
  "mockBoundaryPreserved",
  "noExecution",
  "noSecretValues",
  "noRawPayload"
]) if (output.chainState?.[key] !== true) problems.push(`chainState.${key} must be true`);
if (pkg.scripts?.["report:twii-pre-execution-stopline-chain-convergence-gate"] !== `node ${reportPath}`) problems.push(`${packagePath} missing report script`);
if (pkg.scripts?.["check:twii-pre-execution-stopline-chain-convergence-gate"] !== "node scripts/check-twii-pre-execution-stopline-chain-convergence-gate.mjs") problems.push(`${packagePath} missing check script`);
for (const phrase of [
  "TWII Pre-Execution Stopline Chain Convergence Gate",
  "twii_pre_execution_stopline_chain_convergence_gate_ready_no_execution",
  "pre_execution_stopline_chain_converged_execution_still_blocked",
...
for (const phrase of [
  "scripts/check-twii-pre-execution-stopline-chain-convergence-gate.mjs",
  "name: \"twii-pre-execution-stopline-chain-convergence-gate\"",
  "\"twii-pre-execution-stopline-chain-convergence-gate\""
]) if (!reviewGate.includes(phrase)) problems.push(`${reviewGatePath} missing ${phrase}`);
for (const [filePath, text] of [[gatePath, gateText], [docPath, doc], ["pre-execution stopline chain convergence stdout", run.stdout ?? ""]]) {
