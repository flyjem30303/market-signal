...
const problems = [];
const reportPath = "scripts/report-twii-operator-execution-packet-chain-convergence-gate.mjs";
const docPath = "docs/TWII_OPERATOR_EXECUTION_PACKET_CHAIN_CONVERGENCE_GATE.md";
...
const output = parseJson(run.stdout ?? "", "operator execution packet chain convergence stdout");

if (run.status !== 0) problems.push("operator execution packet chain convergence report must exit 0");
if (output.status !== "twii_operator_execution_packet_chain_convergence_gate_ready_no_execution") problems.push("status mismatch");
if (output.outcome !== "operator_execution_packet_chain_converged_execution_still_blocked") problems.push("outcome mismatch");
if (output.nextPMRoute !== "twii_explicit_operator_go_no_go_decision_preparation_alignment_gate") problems.push("nextPMRoute mismatch");
...
for (const key of [
  "externalToServerPreexecutionChainConverged",
  ...
  "explicitOperatorGoNoGoDecisionPreparationAlignmentPreparedAsNextRoute",
  "mockBoundaryPreserved",
  "noExecution",
  "noSecretValues",
  "noRawPayload"
]) if (output.chainState?.[key] !== true) problems.push(`chainState.${key} must be true`);
if (pkg.scripts?.["report:twii-operator-execution-packet-chain-convergence-gate"] !== `node ${reportPath}`) problems.push(`${packagePath} missing report script`);
if (pkg.scripts?.["check:twii-operator-execution-packet-chain-convergence-gate"] !== "node scripts/check-twii-operator-execution-packet-chain-convergence-gate.mjs") problems.push(`${packagePath} missing check script`);
for (const phrase of [
  "TWII Operator Execution Packet Chain Convergence Gate",
  "twii_operator_execution_packet_chain_convergence_gate_ready_no_execution",
  "operator_execution_packet_chain_converged_execution_still_blocked",
...
for (const phrase of [
  "TWII Operator Execution Packet Chain Convergence Gate",
  "twii_operator_execution_packet_chain_convergence_gate_ready_no_execution",
  "twii_explicit_operator_go_no_go_decision_preparation_alignment_gate"
]) if (!status.includes(phrase)) problems.push(`${statusPath} missing ${phrase}`);
for (const phrase of [
  "scripts/check-twii-operator-execution-packet-chain-convergence-gate.mjs",
  "name: \"twii-operator-execution-packet-chain-convergence-gate\"",
  "\"twii-operator-execution-packet-chain-convergence-gate\""
]) if (!reviewGate.includes(phrase)) problems.push(`${reviewGatePath} missing ${phrase}`);
for (const [filePath, text] of [[gatePath, gateText], [docPath, doc], ["operator execution packet chain convergence stdout", run.stdout ?? ""]]) {
